...
from tensorflow.keras.layers import Dense, Flatten, Conv2D, MaxPooling2D, Dropout, BatchNormalization, Activation, \
    GlobalAveragePooling2D, Conv3D, MaxPooling3D, GlobalAveragePooling3D, Reshape, Lambda

import time
import os

from tensorflow.python.ops.gen_array_ops import quantized_instance_norm_eager_fallback
from Tool.Helper import Counter
...
class Model:
    ...
    def _build_model(self):
        ...
        self.private_move_model.add(
            Conv3D(32, (2, 3, 3), strides=(1, 2, 2), input_shape=self.input_shape, name='conv1'))
        self.private_move_model.add(Activation('relu'))
        self.private_move_model.add(
            Conv3D(48, (2, 3, 3), strides=(1, 1, 1), input_shape=self.input_shape, name='conv2'))
        self.private_move_model.add(Activation('relu'))
        self.private_move_model.add(
            Conv3D(64, (2, 3, 3), strides=(1, 1, 1), input_shape=self.input_shape, name='conv3'))
        self.private_move_model.add(Activation('relu'))
...
class Q_Learn_Model:
    ...
    def load_model(self):
        self.act_model = Q_table(self.act_dim)
        self.move_model = Q_table(4)

    def predict(self, state):
        return self.move_model.getAction(state), self.act_model.getAction(state)
...
class Q_table:
    ...
    def getAction(self, state):
        action_value = Counter()
        for action in range(self.act_dim):
            action_value[action] = self.getQValue(state, action)
        return action_value.argMax()

    def getStateQValue(self, state):
        return max(self.getQValue(state, action) for action in range(self.act_dim))

    def getAllAction(self):
        return [action for action in range(self.act_dim)]

    def setQValue(self, state, action, new_value):
        self.Q_Value[(state, action)] = new_value
